...
export function FeaturedProject({ project, headingId, compact = false }: FeaturedProjectProps) {
  const highlights = compact ? project.highlights?.slice(0, 2) : project.highlights?.slice(0, 3);
  ...
  return (
    <article
      className={cn(
        ...
        compact ? 'gap-5 p-5 md:p-6' : 'gap-8 p-6 md:p-8',
      )}
    >
      <div>
        ...
        <p
          className={cn(
            'max-w-2xl',
            compact ? 'mb-5 line-clamp-2 leading-7' : 'mb-3 text-lg leading-8',
          )}
        >
          {project.description}
        </p>
        {!compact && (
          <p className="text-on-ink-muted mb-6 max-w-2xl leading-7">{project.buildDetails}</p>
        )}
        <div
